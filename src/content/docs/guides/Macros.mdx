...
<Aside>Also you can omit the parenthesis and just write `#[property(default)]`, it will use `Default` trait to do this.</Aside> 
...
<Aside>
The functions which used to verify fields should have such signature where `T` is type of verifying field.
``` rust
fn(Option<&T>) -> Result<(), Box<dyn std::error::Error>
```
</Aside>
